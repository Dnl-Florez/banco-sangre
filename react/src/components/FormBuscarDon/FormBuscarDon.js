...
function FormBuscarDon(props){
	...
	const update = async (e) => {
		e.preventDefault()
		await axios.put(URI+nroDoc_don, {
			"nombre_don": nombre, 
	...
	const getDonByNroDoc = async () => {
		const res = await axios.get(URI+nroDoc_don)
		setNombre(res.data.nombre_don)
		setApellido(res.data.apellido_don)
		setTipoDoc(res.data.tipoDoc_don)
		setNroDoc(res.data.nroDoc_don)
		setGenero(res.data.genero_don)
		setTel(res.data.tel_don)
		setEdad(res.data.edad_don)
		setGs(res.data.gs_don)
		setRh(res.data.rh_don)
		setEps(res.data.eps_don)
		setCantDon(res.data.cantDon_don)
		setFecha(res.data.fechaDon_don)
	};

	const closeEdit = () => {
		setOpenBuscarDon(true);
		setOpenEditarDon(false);
	}
